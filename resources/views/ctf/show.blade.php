...
<head>
    ...
    <title>{{ $ctf->name }} - Cyber Infinity</title>
...
<body class="bg-gray-50">
            ...
            <header class="bg-white shadow-sm border-b border-gray-200">
                <div class="px-6 py-4">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            <a href="{{ route('ctf.index') }}" class="mr-4 text-gray-600 hover:text-gray-900">
                                <i class="fas fa-arrow-left"></i>
                            </a>
                            <div>
                                <h1 class="text-2xl font-bold text-gray-900">{{ $ctf->name }}</h1>
                                <div class="flex items-center gap-2 mt-1">
                                    ...
                                    <span class="text-sm text-gray-500">
                                        {{ $ctf->start_time->format('d M Y H:i') }} - {{ $ctf->end_time->format('d M Y H:i') }}
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex-1">
                            ...
                            <div class="bg-white rounded-xl shadow-sm border border-gray-100 mb-6">
                                <div class="p-6">
                                    ...
                                    <p class="text-gray-700 leading-relaxed">{{ $ctf->description }}</p>
                                </div>
                            </div>
                                    ...
                                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 mb-6">
                                        <div class="p-6">
                                            <div class="flex items-center justify-between mb-4">
                                                ...
                                                @if($userStats['solved'] > 0)
                                                    <div class="flex items-center text-green-600">
                                                        <i class="fas fa-check-circle mr-2"></i>
                                                        <span class="font-medium">{{ $userStats['solved'] }} soal selesai</span>
                                                    </div>
                                                @endif
                                            </div>
                            ...
                            @if($challenges->count() > 0)
                                @foreach($challenges as $category => $categorysChallenges)
                                    <div class="mb-8">
                                        <div class="flex items-center mb-4">
                                            <h2 class="text-xl font-bold text-gray-900 mr-4">{{ $category }}</h2>
                                            <div class="flex-1 h-px bg-gray-200"></div>
                                            <span class="ml-4 text-sm text-gray-500">{{ $categorysChallenges->count() }} soal</span>
                                        </div>
                                        
                                        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                                            @foreach($categorysChallenges as $challenge)
                                                @php
                                                    $isSolved = in_array($challenge->id, $userSubmissions);
                                                    $userHintsForChallenge = $userHints[$challenge->id] ?? [];
                                                @endphp
                                                
                                                <div class="challenge-card bg-white rounded-lg border-2 {{ $isSolved ? 'solved-challenge' : 'border-gray-200 hover:border-blue-300' }} cursor-pointer overflow-hidden"
                                                    ...
                                                    <div class="challenge-content p-4">
                                                        <div class="flex items-center justify-between mb-2">
                                                            <h3 class="text-lg font-bold text-gray-900 truncate">{{ $challenge->title }}</h3>
                                                            @if($isSolved)
                                                        ...
                                                        <div class="flex items-center justify-between text-xs text-gray-500">
                                                            ...
                                                            @if($challenge->hints && count($challenge->hints) > 0)
                                                                <span>
                                                                    <i class="fas fa-lightbulb mr-1"></i>
                                                                    {{ count($challenge->hints) }} hints
                                                                </span>
                                                            @endif
                                                        </div>
                                                    </div>
                        ...
                        <div class="lg:w-80">
                            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sticky top-6">
                                <div class="flex items-center justify-between mb-4">
                                    ...
                                    <a href="{{ route('ctf.leaderboard', $ctf) }}" class="text-blue-600 hover:text-blue-700 text-sm">
                                        Lihat Semua
                                    </a>
                                </div>
                                            ...
                                            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 cursor-pointer transition-colors duration-200"
                                                ...
                                                <div class="flex items-center">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div class="text-gray-500 text-xs">{{ $user->solved_challenges }} solves</div>
                                                    </div>
                                                </div>
                                                <div class="text-blue-600 font-bold">{{ $user->total_points }}</div>
                                            </div>
                                        @endforeach
                                    </div>
                                @else
    ...
    <script>
        const challenges = @json($challenges->flatten());
        const userSubmissions = @json($userSubmissions);
        const userHints = @json($userHints);
        const ctfId = {{ $ctf->id }};
        ...
        function openChallengeModal(challengeId) {
            ...
            let modalContent = `
                ...
                <div class="p-6">
                    ...
                    ${!isSolved && {{ $ctf->isActive() ? 'true' : 'false' }} ? `
                        <div class="mb-6">
                            <h3 class="text-lg font-bold text-gray-900 mb-3">
