...
<head>
    ...
    <title>Edit CTF Event - Cyber Infinity</title>
            ...
            <header class="bg-white shadow-sm border-b border-gray-200">
                <div class="px-6 py-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <h1 class="text-2xl font-bold text-gray-900">Edit CTF Event</h1>
                            <p class="text-sm text-gray-600">Edit event Capture The Flag - {{ $ctf->name }}</p>
                        </div>
            ...
            <main class="flex-1 p-6 bg-gray-50">
                <div class="max-w-2xl mx-auto">
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                        <div class="p-6">
                            <form action="{{ route('admin.ctf.update', $ctf) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="mb-6">
                                    ...
                                    <input type="text" id="name" name="name" value="{{ old('name', $ctf->name) }}" 
                                ...
                                <div class="mb-6">
                                    ...
                                    <textarea id="description" name="description" rows="4" 
                                              ...
                                              placeholder="Jelaskan event CTF, aturan, dan tujuan..." required>{{ old('description', $ctf->description) }}</textarea>
                                    @error('description')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                @if($ctf->banner_image)
                                    <div class="mb-4">
                                        ...
                                        <div class="relative">
                                            <img src="{{ Storage::url($ctf->banner_image) }}" alt="Current Banner" class="w-full h-32 object-cover rounded-lg border border-gray-200">
                                ...
                                <div class="mb-6">
                                    <label for="banner_image" class="block text-sm font-medium text-gray-700 mb-2">
                                        {{ $ctf->banner_image ? 'Ganti Banner' : 'Banner Image' }} <span class="text-gray-400">(Opsional)</span>
                                    </label>
                                    <div class="flex items-center space-x-4">
                                    ...
                                    @error('banner_image')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                    <p class="mt-1 text-xs text-gray-500">{{ $ctf->banner_image ? 'Kosongkan jika tidak ingin mengubah banner' : 'Upload banner untuk event CTF (rekomendasi: 1920x600px)' }}</p>
                                </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                    <div>
                                        ...
                                        <input type="datetime-local" id="start_time" name="start_time" value="{{ old('start_time', $ctf->start_time->format('Y-m-d\TH:i')) }}" 
                                    ...
                                    <div>
                                        ...
                                        <input type="datetime-local" id="end_time" name="end_time" value="{{ old('end_time', $ctf->end_time->format('Y-m-d\TH:i')) }}" 
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                    <div>
                                        ...
                                        <select id="status" name="status" 
                                            ...
                                            <option value="draft" {{ old('status', $ctf->status) == 'draft' ? 'selected' : '' }}>Draft</option>
                                            <option value="active" {{ old('status', $ctf->status) == 'active' ? 'selected' : '' }}>Active</option>
                                            <option value="inactive" {{ old('status', $ctf->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                                        </select>
                                        @error('status')
                                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    ...
                                    <div>
                                        ...
                                        <input type="number" id="max_participants" name="max_participants" value="{{ old('max_participants', $ctf->max_participants) }}" min="1"
                                ...
                                <!-- Rules -->
                                <div class="mb-6" x-data="{ rules: {{ json_encode(old('rules', $ctf->rules ?? [])) }} }">
                                ...
                                <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200">
                                    ...
                                    <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors duration-200">
                                        <i class="fas fa-save mr-2"></i>Update CTF Event
                                    </button>
                                </div>
                            </form>
                        </div>
