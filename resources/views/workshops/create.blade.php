...
<head>
    ...
    <title>Tambah Workshop - Cyber Infinity</title>
...
<body class="bg-white">
        ...
        <aside id="sidebar" class="w-64 bg-white shadow-sm border-r border-gray-200 fixed inset-y-0 left-0 z-50 transform -translate-x-full transition-transform duration-300 lg:!transform-none lg:!translate-x-0 lg:!static lg:!inset-0">
            <div class="flex flex-col h-full">
                ...
                <nav class="flex-1 px-4 py-4 space-y-2">
                    ...
                    <a href="{{ route('workshops.create') }}" class="flex items-center px-3 py-2 text-sm font-medium text-blue-600 bg-blue-50 rounded-lg">
            ...
            <header class="bg-white shadow-sm border-b border-gray-200">
                <div class="px-6 py-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <h1 class="text-2xl font-bold text-gray-900">Tambah Workshop</h1>
                            <p class="text-sm text-gray-600">Buat workshop baru untuk peserta</p>
                        </div>
                ...
                <div class="max-w-4xl mx-auto">
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                        <div class="p-6 border-b border-gray-100">
                            ...
                            <p class="text-sm text-gray-600 mt-1">Lengkapi informasi workshop yang akan dibuat</p>
                        </div>
                        
                        <form method="POST" action="{{ route('workshops.store') }}" class="p-6 space-y-6">
                            @csrf
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select id="activity_type_select" onchange="toggleCustomActivityType()" 
                                        ...
                                        <option value="bootcamp" {{ old('activity_type') == 'bootcamp' ? 'selected' : '' }}>Bootcamp</option>
                                        <option value="training" {{ old('activity_type') == 'training' ? 'selected' : '' }}>Training</option>
                                        <option value="custom" {{ old('activity_type') && !in_array(old('activity_type'), ['workshop', 'bootcamp', 'training']) ? 'selected' : '' }}>🎨 Custom (Ketik Sendiri)</option>
                                    </select>
                                    
                                    <!-- Hidden input for actual form submission -->
                                    <input type="hidden" id="activity_type" name="activity_type" value="{{ old('activity_type') }}" required>
                                    ...
                                    <div id="custom_activity_wrapper" class="mt-3" style="display: none;">
                                        <input type="text" id="custom_activity_input" 
                                               ...
                                               value="{{ old('activity_type') && !in_array(old('activity_type'), ['workshop', 'bootcamp', 'training']) ? old('activity_type') : '' }}"
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea id="requirements" name="requirements" rows="3"
                                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('requirements') border-red-500 @enderror">{{ old('requirements') }}</textarea>
                                    @error('requirements')
                                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                            ...
                            <div class="flex justify-end space-x-4 pt-6 border-t border-gray-100">
                                ...
                                <button type="submit" 
                                    ...
                                    <i class="fas fa-save mr-2"></i>Simpan Workshop
                                </button>
                            </div>
                        </form>
                    </div>
    ...
    <script>
        ...
        function confirmCancel() {
            Swal.fire({
                title: 'Batalkan Pembuatan Workshop?',
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Check if we have old custom value
            const oldActivityType = '{{ old("activity_type") }}';
            const standardTypes = ['workshop', 'bootcamp', 'training'];
            
            if (oldActivityType && !standardTypes.includes(oldActivityType)) {
