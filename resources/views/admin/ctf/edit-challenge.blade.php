...
<head>
    ...
    <title>Edit Soal CTF - {{ $challenge->title }} - Cyber Infinity</title>
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            <div class="flex items-center mb-2">
                                ...
                                <h1 class="text-2xl font-bold text-gray-900">Edit Soal CTF</h1>
                            </div>
                            <p class="text-sm text-gray-600">Edit soal {{ $challenge->title }} - {{ $ctf->name }}</p>
                        </div>
            ...
            <main class="flex-1 p-6 bg-gray-50">
                <div class="max-w-4xl mx-auto">
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                        <div class="p-6">
                            <form action="{{ route('admin.ctf.challenges.update', [$ctf, $challenge]) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                    <div>
                                        ...
                                        <select id="category" name="category" 
                                            ...
                                            <option value="OSINT" {{ old('category', $challenge->category) == 'OSINT' ? 'selected' : '' }}>OSINT</option>
                                            <option value="Reverse" {{ old('category', $challenge->category) == 'Reverse' ? 'selected' : '' }}>Reverse Engineering</option>
                                            <option value="Pwn" {{ old('category', $challenge->category) == 'Pwn' ? 'selected' : '' }}>Pwn</option>
                                            <option value="Linux" {{ old('category', $challenge->category) == 'Linux' ? 'selected' : '' }}>Linux</option>
                                            <option value="Network" {{ old('category', $challenge->category) == 'Network' ? 'selected' : '' }}>Network</option>
                                            <option value="Mobile" {{ old('category', $challenge->category) == 'Mobile' ? 'selected' : '' }}>Mobile</option>
                                            <option value="Hardware" {{ old('category', $challenge->category) == 'Hardware' ? 'selected' : '' }}>Hardware</option>
                                        </select>
                                        @error('category')
                                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    ...
                                    <div>
                                        ...
                                        <input type="number" id="points" name="points" value="{{ old('points', $challenge->points) }}" min="1" max="1000"
                                ...
                                <div class="mb-6">
                                    ...
                                    <textarea id="description" name="description" rows="5" 
                                              ...
                                              placeholder="Jelaskan soal CTF, konteks, dan petunjuk yang diperlukan..." required>{{ old('description', $challenge->description) }}</textarea>
                                    @error('description')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-6">
                                    ...
                                    <div class="flex items-center space-x-4 mb-2">
                                        <input type="text" id="flag" name="flag" value="{{ old('flag', $challenge->flag) }}" 
                                        ...
                                        <div class="flex items-center">
                                            <input type="checkbox" id="case_sensitive" name="case_sensitive" value="1" {{ old('case_sensitive', $challenge->case_sensitive) ? 'checked' : '' }}
                                ...
                                <!-- Current Files -->
                                @if($challenge->files && count($challenge->files) > 0)
                                    <div class="mb-4">
                                        <label class="block text-sm font-medium text-gray-700 mb-2">File Saat Ini</label>
                                        <div class="space-y-2">
                                            @foreach($challenge->files as $index => $file)
                                                <div class="flex items-center justify-between p-3 bg-blue-50 border border-blue-200 rounded-lg">
                                                    <div class="flex items-center">
                                                        <i class="fas fa-file text-blue-600 mr-2"></i>
                                                        <span class="text-sm font-medium">{{ $file['name'] ?? 'File #' . ($index + 1) }}</span>
                                                        <span class="text-xs text-gray-500 ml-2">({{ isset($file['size']) ? number_format($file['size'] / 1024, 1) . ' KB' : 'Unknown size' }})</span>
                                                    </div>
                                                    <a href="{{ $file['path'] ?? '#' }}" target="_blank" class="text-blue-600 hover:text-blue-800 text-sm">
                                ...
                                <div class="mb-6" x-data="{ files: [] }">
                                    <label class="block text-sm font-medium text-gray-700 mb-2">
                                        {{ $challenge->files && count($challenge->files) > 0 ? 'Ganti/Tambah File' : 'File Attachment' }} <span class="text-gray-400">(Opsional)</span>
                                    </label>
                                    ...
                                    <p class="mt-1 text-xs text-gray-500">{{ $challenge->files && count($challenge->files) > 0 ? 'File baru akan ditambahkan ke file yang sudah ada' : 'Upload file yang dibutuhkan untuk menyelesaikan soal (zip, txt, dll)' }}</p>
                                </div>

                                <!-- Hints -->
                                <div class="mb-6" x-data="{ hints: {{ json_encode(old('hints', $challenge->hints ?? [])) }} }">
                                    <label class="block text-sm font-medium text-gray-700 mb-2">
                                        ...
                                        <template x-for="(hint, index) in hints" :key="index">
                                            <div class="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                                                ...
                                                <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                                    <input type="text" :name="`hints[${index}][title]`" x-model="hints[index].title"
                                                    ...
                                                    <input type="number" :name="`hints[${index}][cost]`" x-model="hints[index].cost"
                                                           placeholder="Cost (points)" min="1"
                                                           class="px-3 py-2 border border-yellow-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 text-sm">
                                                    <textarea :name="`hints[${index}][content]`" x-model="hints[index].content" rows="2"
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                    <div>
                                        ...
                                        <select id="status" name="status" 
                                            ...
                                            <option value="hidden" {{ old('status', $challenge->status) == 'hidden' ? 'selected' : '' }}>Hidden</option>
                                        </select>
                                        @error('status')
                                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    ...
                                    <div>
                                        ...
                                        <input type="number" id="max_attempts" name="max_attempts" value="{{ old('max_attempts', $challenge->max_attempts) }}" min="1"
                                ...
                                <div class="mb-6 p-4 bg-gray-50 rounded-lg">
                                    <h3 class="text-sm font-medium text-gray-900 mb-3">Statistik Soal</h3>
                                    <div class="grid grid-cols-3 gap-4 text-center">
                                        <div>
                                            <div class="text-lg font-bold text-blue-600">{{ $challenge->solve_count ?? 0 }}</div>
                                            <div class="text-xs text-gray-500">Solves</div>
                                        </div>
                                        <div>
                                            <div class="text-lg font-bold text-green-600">{{ $challenge->submissions()->where('status', 'correct')->count() }}</div>
                                            <div class="text-xs text-gray-500">Correct</div>
                                        </div>
                                        <div>
                                            <div class="text-lg font-bold text-red-600">{{ $challenge->submissions()->where('status', 'wrong')->count() }}</div>
                                ...
                                <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200">
                                    ...
                                    <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors duration-200">
                                        <i class="fas fa-save mr-2"></i>Update Soal CTF
                                    </button>
                                </div>
                            </form>
                        </div>
